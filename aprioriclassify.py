...
def getapriori(dataset, minsup):      # 数据集，支持度，置信度
    result = []
    C1 = getC1(dataset)
    ...
    for item in freqiterms:
        for i in range(len(item)):
            ...
            if confidence >= miniconfidence:
                rules.append([itemA] + [B] + [confidence])
    return rules
...
class CBAclassifer:
    ...
    def preprocess(self):
        ...
        for index, flag in enumerate(self.flags):
            if flag == 1:
                ...
                self.borders.append(common.discrete(self.dataset, index, typename))
            # todo 真正的2 2应该是属性值 应该建立一个对应字典 下面的是3是类别值
            elif flag == 2:
    ...
    def addsupport(self):
        ...
        for i in range(len(self.rules)):
            items = self.rules[i][0] + [self.rules[i][1]]
    ...
    # 根据rules的2,3也就是confidence和support进行排序
    def sortprune(self):
        tmp = sorted(self.rules, key=lambda rule: rule[3], reverse=True)
        self.rules = sorted(tmp, key=lambda k: k[2], reverse=True)
    ...
    def predict(self, test):
        if len(self.rules) == 0:
            print("you need to learn first")
            return
        for k, t_item in enumerate(test):
            # 转化成固定值
            for i, t in enumerate(t_item[:-1]):
                for j, border in enumerate(self.borders[i]):
                    if float(t) <= border:
                        test[k][i] = feats[i] + str(j+1)
                        break
        correct_num = 0
        for t_item in test:
            for rule in self.rules:
                flag = True
                for r in rule[0]:              # 验证是否符合rule的条件
                    if r not in t_item:
                        flag = False
                        break
                if flag:
                    class_guess = self.ynames[int(rule[1][-1])-1]
                    if class_guess == t_item[-1]:
                        correct_num += 1
                    break
        result1 = (correct_num / len(test))
        return result1
...
for index in range(10):
    ...
    cba.learn()
    apriori_result.append(cba.predict(test))
    # C4.5算法对比
    c45 = c4_5.C4_5(cba.dataset[:], cba.feats[:-1])
    # 对test标签进行一下修改
    for i in range(len(test)):
        for j, yn in enumerate(cba.ynames):
            if test[i][-1] == yn:
                test[i][-1] = 'class' + str(j + 1)
                break
    c45_result.append(c45.judge(test))
# 画图比较
plt.plot([i for i in range(10)], apriori_result, 'r', label='apriori')
plt.plot([i for i in range(10)], c45_result, 'b', label='c4.5')
plt.xlabel("times")
plt.ylabel("accurate rate")
plt.title("Comparison of Two Classifier")
plt.legend()
plt.show()
